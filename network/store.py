...
class Store:
    def __init__(self, total_num_classes, items_per_class, shuffle=False):
        ...
        # self.store1 = [[] for _ in range(self.total_num_classes)]
        # self.num = [0 for _ in range(self.total_num_classes)]

    def add(self, items, class_ids):
        for idx, class_id in enumerate(class_ids):
            self.store[class_id].append(items[idx])
            # self.store1[class_id].append(items[idx])

        # for idx,item in enumerate(self.store1):
        #     self.store1[idx] = self.store1[idx][-self.items_per_class:]
    ...
    def retrieve(self, class_id):
        if class_id != -1:
            ...
            return all_items
    #
    # def retrieve1(self, class_id):
    #     if class_id != -1:
    #         items = []
    #         for item in self.store1[class_id]:
    #             items.extend(list(item[-self.items_per_class:]))
    #         if self.shuffle:
    #             random.shuffle(items)
    #         return items
    #     else:
    ...
    #         for i in range(self.total_num_classes):
    #             items = []
    #             sub_item = self.store1[i][-self.items_per_class:]
    ...
    def reset(self):
        # self.store = [deque(maxlen=self.items_per_class) for _ in range(self.total_num_classes)]
        self.store = [[] for _ in range(self.total_num_classes)]
