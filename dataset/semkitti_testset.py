...
class SemanticKITTI(torch_data.IterableDataset):
    ...
    def collate_fn(self, batch):
        ...
        inputs = {}
        inputs['xyz'] = [torch.from_numpy(t).float() for t in flat_inputs[:num_layers]]
        inputs['neigh_idx'] = [torch.from_numpy(t).long() for t in flat_inputs[num_layers:2 * num_layers]]
        inputs['sub_idx'] = [torch.from_numpy(t).long() for t in flat_inputs[2 * num_layers:3 * num_layers]]
        inputs['interp_idx'] = [torch.from_numpy(t).long() for t in flat_inputs[3 * num_layers:4 * num_layers]]
